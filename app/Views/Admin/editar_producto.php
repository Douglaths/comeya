                        ...
                        <div class="card-body">
                            <form action="<?= base_url('admin/productos/update/' . $producto['id']) ?>" method="POST" enctype="multipart/form-data">
                                ...
                                <div class="mb-3">
                                    ...
                                    <textarea class="form-control" id="descripcion" name="descripcion" rows="3"><?= esc($producto['descripcion']) ?></textarea>
                                </div>
                            ...
                            <div class="preview-product">
                                <div class="product-image bg-light rounded mb-3" id="imagePreview" style="height: 150px; display: flex; align-items: center; justify-content: center; overflow: hidden;">
                                    <?php if ($producto['imagen']): ?>
                                        <img src="<?= base_url('public/uploads/' . $producto['imagen']) ?>" style="width: 100%; height: 100%; object-fit: cover;">
                                    <?php else: ?>
                                        <span class="text-muted">Sin imagen</span>
                                    <?php endif; ?>
                                </div>
                                <h6 class="product-name"><?= esc($producto['nombre']) ?></h6>
                                <p class="product-description text-muted small"><?= esc($producto['descripcion']) ?></p>
                                <h5 class="product-price text-primary">$<?= number_format($producto['precio'], 0) ?></h5>
                            </div>
                        </div>
                    </div>
                </div>
